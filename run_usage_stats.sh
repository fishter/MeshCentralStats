...
${FIND_BIN} ${INPUT_PATH} -name meshcentral-events.db\* -newermt "$(${DATE_BIN} --date='yesterday')" -exec ${PATH}/AnalyseMeshCentralEventsdb_v3.py --before=${BEFORE} --since=${SINCE} --measurement=dec -o ${OUTPUT_FILE} {} +
...
#    BEFORE=$(${DATE_BIN} +"%Y-%m-%d" --date="today") # does not include today
    ...
    ${FIND_BIN} ${INPUT_PATH} -name meshcentral-events.db\* -newermt "$(${DATE_BIN} --date='last month')" -exec ${PATH}/AnalyseMeshCentralEventsdb_v3.py --before=${BEFORE} --since=${SINCE} --measurement=dec -o ${OUTPUT_FILE} {} +
    if [ $? == 0 ]
    then
