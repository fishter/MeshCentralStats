...
SCRIPT_NAME=${HOMEPATH}/MeshCentralStats/AnalyseMeshCentralEventsdb_v4.py
...
SCRIPT_OPTIONS="--before=${BEFORE} --since=${SINCE} -mdec -g60 -o ${OUTPUT_FILE}"
#echo  ${INPUT_PATH} -name meshcentral-events.db\* -newermt "$(date --date='yesterday')" -exec ${SCRIPT_NAME} ${SCRIPT_OPTIONS} {} +
...
# and delete all the daily files from the before previous month.
if [ $(date +"%d") == 01 ]
then
...
#    BEFORE=$(${DATE_BIN} +"%Y-%m-%d" --date="today") # does not include today
    ...
    # echo ${INPUT_PATH} -name meshcentral-events.db\* -newermt "$(date --date='last month')" -exec ${SCRIPT_NAME} ${SCRIPT_OPTIONS} {} +
    find ${INPUT_PATH} -name meshcentral-events.db\* -newermt "$(date --date='last month')" -exec ${SCRIPT_NAME} ${SCRIPT_OPTIONS} {} +
    if [ $? == 0 ]
    then
        rm ${OUTPUT_PATH}/NetUsage_Daily_$(date +'%Y-%m' --date='-6 weeks')-*.txt 2> /dev/null
    fi
fi
